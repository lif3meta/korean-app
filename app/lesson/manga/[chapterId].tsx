...
// Try to load local manga images (generated by scripts/generate-manga-images.mjs)
let mangaImages: Record<string, any> = {};
try {
  mangaImages = require('@/data/mangaImages').mangaImages;
} catch {
  // No local images yet, will fall back to remote
}
...
export default function MangaReaderScreen() {
  ...
  for (const series of mangaSeries) {
    ...
    if (idx !== -1) {
      chapter = series.chapters[idx];
      if (idx + 1 < series.chapters.length) {
        ...
        <View style={styles.endCard}>
          ...
          {nextChapter ? (
            <TouchableOpacity
              ...
              onPress={() => router.replace(`/lesson/manga/${nextChapter!.id}`)}
            >
              <Ionicons name="arrow-forward" size={16} color="#fff" />
              <Text style={styles.endButtonText}>Next: {nextChapter.title}</Text>
            </TouchableOpacity>
          ) : null}
          <TouchableOpacity
            style={[styles.endButton, nextChapter ? styles.endButtonSecondary : null]}
            onPress={() => router.back()}
          >
            <Text style={[styles.endButtonText, nextChapter ? styles.endButtonSecondaryText : null]}>
              Back to Stories
            </Text>
          </TouchableOpacity>
        </View>
  ...
  return (
    <View style={styles.panelContainer}>
      ...
      <View style={styles.imageContainer}>
        {localImage ? (
          <Image source={localImage} style={styles.panelImage} resizeMode="cover" />
        ) : (
          <CachedImage uri={remoteUrl} style={styles.panelImage} />
        )}
      </View>
